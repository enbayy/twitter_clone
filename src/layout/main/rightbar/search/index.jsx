...
export default function Search() {
    ...
    return (
        ...
        >
            <label className="h-[43px] rounded-full bg-[color:var(--background-third)] w-full relative group border border-transparent focus-within:bg-[color:var(--background-primary)] focus-within:border-[#1d9bf0]">
                ...
                {(query && focus) && (
                    <button
                        type="button"
                        onClick={() => setQuery('')}
                        className="w-[22px] h-[22px] rounded-full bg-[color:var(--color-primary)] flex items-center justify-center text-black min-w-[22px] absolute right-3 top-1/2 -translate-y-1/2"><svg viewBox="0 0 15 15" width={10} height={10}>
                            <path
                                fill="currentColor"
            ...
            {focus && (
                <div
                    className="absolute top-full -left-px -translate-y-1 w-[350px] bg-[color:var(--background-primary)] shadow-box max-h-[calc(80vh-53px)] rounded-lg text-center min-h-[100px]">
